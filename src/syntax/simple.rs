    ...
    match name {
        "sql" => Some(SimpleRules {
            keywords: &[
                ...
                "NOT", "IN", "IS", "NULL", "AS", "ORDER", "BY", "GROUP", "HAVING",
                "LIMIT", "OFFSET", "UNION", "ALL", "DISTINCT", "BETWEEN", "LIKE", "EXISTS",
                "CASE", "WHEN", "THEN", "ELSE", "END", "BEGIN", "COMMIT", "ROLLBACK",
                "TRANSACTION", "PRIMARY", "KEY", "FOREIGN", "REFERENCES", "CONSTRAINT",
                "DEFAULT", "CHECK", "UNIQUE", "IF", "REPLACE", "TRIGGER", "FUNCTION",
                "PROCEDURE", "RETURNS", "RETURN", "DECLARE", "CURSOR", "FETCH", "OPEN",
                "CLOSE", "WITH", "RECURSIVE", "TEMPORARY", "TEMP", "CASCADE", "RESTRICT",
                "ASC", "DESC", "EXPLAIN", "ANALYZE", "GRANT", "REVOKE", "SCHEMA",
                "INT", "INTEGER", "BIGINT", "SMALLINT", "FLOAT", "DOUBLE", "DECIMAL",
                "NUMERIC", "CHAR", "VARCHAR", "TEXT", "BOOLEAN", "BOOL", "DATE", "TIME",
                "TIMESTAMP", "SERIAL", "AUTOINCREMENT", "TRUE", "FALSE",
                "COUNT", "SUM", "AVG", "MIN", "MAX", "COALESCE", "CAST",
                "ADD", "COLUMN", "RENAME", "TO", "TRUNCATE", "EXCEPT", "INTERSECT",
            ],
            line_comment: Some("--"),
            block_comment: Some(("/*", "*/")),
            string_delimiters: &['\'', '"'],
...
mod tests {
    ...
    fn sql_line_comment() {
        ...
        assert_eq!(&source[highlights[0].0..highlights[0].1], "-- this is a comment");
    }
    ...
    fn sql_block_comment() {
        ...
        let highlights = compute_simple_highlights(&rules, source);
        assert_eq!(highlights[0].2, "comment");
        assert_eq!(&source[highlights[0].0..highlights[0].1], "/* block comment */");
    }
